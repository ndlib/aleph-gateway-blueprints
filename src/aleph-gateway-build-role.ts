...
export interface IAlephGatewayBuildRoleProps extends RoleProps {
  readonly stages: string[]
  readonly artifactBucket: Bucket
}

export class AlephGatewayBuildRole extends Role {
  constructor(scope: cdk.Construct, id: string, props: IAlephGatewayBuildRoleProps) {
    super(scope, id, props)

    const serviceStackPrefix = scope.node.tryGetContext('serviceStackName') || 'aleph-gateway'
    ...
    this.addToPolicy(
      new PolicyStatement({
        resources: [
          ...
          Fn.sub('arn:aws:ssm:${AWS::Region}:${AWS::AccountId}:parameter/all/oracle-lambda-layer/*'),
    ...
    this.addToPolicy(
      new PolicyStatement({
        resources: [Fn.sub('arn:aws:secretsmanager:${AWS::Region}:${AWS::AccountId}:secret:/all/aleph-gateway-*')],
        actions: [
...
export default AlephGatewayBuildRole
